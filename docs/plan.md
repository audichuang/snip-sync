...
| 6 操作真實 App | `tauri-driver`(WebDriver):Windows 走 msedgedriver、Linux 走 WebKitWebDriver | ✅ Windows / Linux |
| | macOS 的 WKWebView 沒有 WebDriver | ❌ 手動 |
| | 系統匣選單本身 | ❌ 各平台都難以自動化 |

- **CI 比照 aghub 的 ci.yml:** actionlint、format(rustfmt + prettier)、三平台 clippy `-D warnings`
  與 `cargo audit`、前端 lint + 單元測試、三平台 `cargo test --workspace --no-fail-fast`。
  依賴一律 `bun install --frozen-lockfile`。
