...
const sameTarget = (a: DiffTarget, b: DiffTarget) => JSON.stringify(a) === JSON.stringify(b);

/** One open diff at a time, fetched from the pending plan on demand. */
export function useDiff() {
	const { t } = useTranslation();
	const [open, setOpen] = useState<{ target: DiffTarget; patch: string } | null>(null);
	...
	async function toggle(target: DiffTarget) {
		...
		try {
			setOpen({ target, patch: await invoke<string>("diff", { target }) });
		} catch (error: unknown) {
			toast.danger(errorText(t, error));
		}
	}

	const isOpen = (target: DiffTarget) => open !== null && sameTarget(open.target, target);
...
export function DiffButton({
	...
	const { t } = useTranslation();
	return (
		<Button size="sm" variant="ghost" onPress={() => void diff.toggle(target)}>
...
export function DiffBody({ patch }: { patch: string }) {
	const { t } = useTranslation();
	if (!patch.includes("@@")) return <p className="text-sm text-muted">{t("noDiff")}</p>;
	return (
		<div className="overflow-hidden rounded border border-border">
			<PatchDiff
				patch={withGitHeader(patch)}
				disableWorkerPool
				options={{ diffStyle: "unified", overflow: "wrap", disableFileHeader: true }}
			/>
		</div>
	);
}
